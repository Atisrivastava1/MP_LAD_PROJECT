...
MPLADS Sanchalan is an **SIH prototype** for detecting statistically unusual MPLADS projects and helping auditors prioritize investigations using an AI/ML engine. 
...
## Frontend
Built with **Flutter (Web/Mobile)**, providing a fully responsive and cross-platform experience for both Auditors and Data Managers.
The UI boasts a premium, modern design language incorporating glassmorphism, dynamic gradients, and fluid layouts. Key features include:
- **Interactive Dashboards**: Real-time donut charts and risk metrics tracking ongoing and completed projects.
- **Data Quality Assurance**: Automated logic tracking data completeness and logical date ranges before generating ML inferences.
- **Reporting & Exports**: One-click generation of comprehensive CSV extracts for clean data, flagged projects, and overall status reports.
...
## ML Model
The production ML engine utilizes an **Isolation Forest** algorithm to process distinct statistical features derived from raw MPLADS projects (e.g., description length, recommended amount logs, completion delays). 
It outputs a standardized Risk Score (0-100) and categorizes projects into Low, Medium, High, or Critical risk brackets to generate immediate investigation signals for the dashboard. The model is specifically tuned to account for active/ongoing project states to prevent false positives on incomplete data.
...
python run.py
`
This will start the FastAPI server on http://localhost:8000 and automatically load the ML engine artifacts. The Flutter web application can be launched from the rontend/ directory.
...
- **Deployment:** Soumyadeep Nath (Production Deployment and Hosting)
